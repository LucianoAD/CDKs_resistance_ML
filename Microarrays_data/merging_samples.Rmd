...
matrices <- list(
  ...
  merge42_numeric = merge42_numeric,  # Otra matriz de datos numéricos
  merge70_numeric = merge70_numeric,  # Otra matriz numérica
...
crear_listas <- function(matrices) {
  ...
  for (i in seq_along(matrices)) {  # Recorrer cada matriz en la lista
    ...
    assign(paste0("lista", sufijo), df, envir = .GlobalEnv)  # Crear una lista para cada sufijo numérico (lista02, lista42, etc.)
  }
}

# Ejecutar la función para crear las listas
...
Para eliminar los efectos de lote en nuestros datos, aplicaremos tres métodos distintos: Combat, limma y SVA. Cada uno está diseñado para manejar diferentes tipos de variabilidad en los datos de expresión génica.
...
2️⃣ Corrección de lote con limma
El paquete limma ofrece una corrección de batch effect mediante la función removeBatchEffect(). A diferencia de Combat, que modela explícitamente el efecto de lote, limma ajusta la expresión génica utilizando modelos lineales para eliminar la variabilidad técnica sin alterar la señal biológica de interés. Este método es útil cuando ya se han identificado los efectos de lote pero se desea una corrección más flexible, que se pueda integrar en análisis de expresión diferencial.

3️⃣ SVA (Surrogate Variable Analysis)
...
En comparación, Combat logró reducir en mayor medida los efectos de lote. Aunque se observa una clara disminución en la formación de clústeres, algunas muestras de la misma plataforma siguen agrupadas estrechamente. Esto sugiere nuevamente la presencia de fuentes de variabilidad no controladas.

**3)Apliquemos ahora SVA:**

```{r, eval=FALSE}
# Definir modelo con SOLO la condición biológica
mod <- model.matrix(~ condicion, data = metadata_combinada)  # Mantiene la condición biológica
mod0 <- model.matrix(~ 1, data = metadata_combinada)  # Modelo nulo con solo el intercepto

# Determinar el número de variables sustitutas
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = Plataforma, shape = Condicion)) +
  ...
  scale_color_manual(values = colores) +
  scale_shape_manual(values = formas) +
  labs(title = "PCA después de aplicar SVA",
       x = "PC1",
       y = "PC2") +
  theme_minimal()
```

En este gráfico de PCA, agregué el nombre de las muestras para identificar mejor aquellas que presentan un comportamiento atípico. Si bien se observa una reducción significativa de los efectos de lote, algunas muestras individuales muestran una gran desviación con respecto al resto. Esto sugiere que podrían tener problemas técnicos o estar introduciendo un alto nivel de ruido en los datos.
En general, la integración de los datos es adecuada, salvo por dos muestras de la plataforma Agilent (GPL20844, en rojo), que siguen mostrando una alta variabilidad no corregida. Dado que su ruido de fondo no pudo ser eliminado, procederemos a remover estas muestras y repetir el análisis.
...
```{r, eval=FALSE}
# Definir modelo con SOLO la condición biológica
mod <- model.matrix(~ condicion, data = metadata_combinada_filtrada)  # Mantiene la condición biológica
...
# Agregar las etiquetas de plataforma y condición
pca_df$Plataforma <- rep(c("GPL20844", "GPL571", "GPL17692", "GPL10558"), 
                         times = c(6, 6, 21, 23))  # Ajustar según el número de muestras por plataforma
...
Observando la escala del gráfico, podemos notar que la variabilidad entre las plataformas se redujo aproximadamente a la mitad tras la eliminación de las dos muestras problemáticas. Además, las muestras ya no muestran un agrupamiento exclusivo por color (plataforma), lo que indica una mejor integración de los datos.

Sin embargo, las muestras de la plataforma Agilent parecen ser más difíciles de integrar con los datos de Affymetrix, a diferencia de las de Illumina, que muestran una mejor adaptación. Dado esto, exploraremos qué sucede si eliminamos por completo la plataforma Agilent y repetimos el análisis.
...
```{r, eval=FALSE}
# Definir modelo con SOLO la condición biológica
mod <- model.matrix(~ condicion, data = metadata_combinada_3p)  # Mantiene la condición biológica
...
pca_df$Plataforma <- rep(c( "GPL571", "GPL17692", "GPL10558"), 
                         times = c(6, 21, 23))  # Ajustar según el número de muestras por plataforma
...
Aunque este también es un buen resultado, observamos que, en comparación con el PCA anterior, la variabilidad no se reduce tanto al eliminar completamente la plataforma Agilent. Además, parece surgir nuevamente un cierto grado de agrupamiento entre algunas muestras de la misma plataforma.
Por esta razón, en principio utilizaremos la matriz corregida en el análisis previo para aplicar los métodos de aprendizaje automático.